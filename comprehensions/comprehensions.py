...
def get_unique_user_ids_visited_page_after_ts(
    ...
    """
    return {record["UserID"] for record in records if record["PageID"] == page_id and record["EventTime"] > ts}
...
def get_region_ids_with_none_replaces_by_default(
    ...
    :return: region ids
    """
    return [r["RegionID"] if r["RegionID"] is not None else 100500 for r in records]
...
def get_keys_if_key_in_keys(
    ...
    """
    return {key for key in keys if key in r.keys()}
